...
void CMathLibMe::M001( int index, int n, double s, double m, double high, double low, double hhv, double llv, int last_state, double last_sar, double last_af, int *state, double *af, double *sar )
{
	if (index >= n) {
		if (index == n)
		{
...
int CMathLibMe::M012( double min, double max, int yLen, int maxSpan, int minSpan, int defCount, double *step, int *digit )
{
	...
	*digit = 0;
	...
	for (i = 15; i >= -6; i--)
	{
		...
		if (divisor < 1.0)
		{
			localObject = digit;localInteger1 = *((int*)localObject);localInteger2 = *((int*)localObject) = *((int*)localObject) + 1;
		}
		...
		if (start)
		{
			if (nTemp < 4)
			{
				...
				localObject = digit;localInteger1 = *((int*)localObject);localInteger2 = *((int*)localObject) = *((int*)localObject) - 1; break;
			}
			if ((nTemp >= 4) && (nTemp <= 6))
			{
				nTemp = 5;
				localObject = step;*((double*)localObject) = *((double*)localObject) + nTemp * divisor; break;
			}
...
double CMathLibMe::M129( int tm_year, int tm_mon, int tm_mday,
                        int tm_hour, int tm_min, int tm_sec, int tm_msec )
{
	struct tm t;
	time_t tn;
	t.tm_sec = tm_sec;
