...
CursorsA CPlotBaseMe::GetCursor()
{
	return m_div->GetChart()->GetCursor();
}

void CPlotBaseMe::SetCursor(CursorsA cursor)
{
...
void CPlotBaseMe::FillRect(CPaintMe *paint, _int64 dwPenColor, const RECT& rect)
{
	...
	//	CChartMe *chart = GetChart();
	//	POINT mp = chart->GetMousePoint();
	//	if (((!m_isPaintingGhost && (mp.y >= m_div->GetTop())) && (mp.y <= m_div->GetBottom())) 
	//		&& ((chart->GetMovingPlot() == this) || (((chart == GetNative()->GetHoveredControl()) && !chart->IsOperating()) && OnSelect())))
	//	{
	//		int a = 0;
	//		int r = 0;
	//		int g = 0;
...
double* CPlotBaseMe::GetLRBandRange(map<int,CPlotMarkMe*> *marks, float *param)
{
	...
	{
		double high = m_dataSource->Get2(i, sIterHigh->second);//v36
		double low = m_dataSource->Get2(i, sIterLow->second);//v35
...
{
	float y1 = PY(value1);//v14
	float y2 = PY(value2);//v13
	float y0 = 0;
	float yA = 0;
	float yB = 0;
	float yC = 0;
	...
	*length = 6;
	...
	retArray[4] = yD;
	retArray[5] = y100;
	//float retArray[] = { y0, yA, yB, yC, yD, y100 };
	return retArray;
}
...
void CPlotBaseMe::Move(const POINT& mp)
{
	...
	{
		int startIndex = m_dataSource->GetRowIndex(m_startMarks[i]->Key);//v24
		int newIndex = 0;
