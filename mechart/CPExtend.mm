...
void P001::OnMoveStart()
{
	...
	if (CPlotBaseMe::m_action != ActionType_NO)
	{
		...
		//m_startMarks[2] = m_marks[2];
		//m_startMarks[3] = m_marks[3];
	}
}
...
void P002::OnMoveStart()
{
	...
	if (m_action != ActionType_NO)
	{
		...
		//base.m_startMarks[0] = base.m_marks[0];
		//base.m_startMarks[1] = base.m_marks[1];
		//base.m_startMarks[2] = base.m_marks[2];
	}
}
...
void P005::Paint(CPaintMe *paint, map<int,CPlotMarkMe*> *pList, _int64 lineColor)
{
	...
	if (size > 0)
	{
		...
		POINT *points = new POINT[7];// { point, point2, point3, point4, point5, point6, point7 };
		points[0] = point;
		points[1] = point2;
		points[2] = point3;
		points[3] = point4;
...
void P009::OnMoveStart()
{
	...
	if (CPlotBaseMe::m_action != ActionType_NO)
	{
		...
		CPlotBaseMe::m_startMarks.insert(make_pair(1, sIterPlot->second));
		//CPlotBaseMe::m_startMarks[0] = CPlotBaseMe::m_marks[0];
		//CPlotBaseMe::m_startMarks[1] = CPlotBaseMe::m_marks[1];
	}
}
...
ActionType P010::GetAction()
{
	...
	if ((int)CPlotBaseMe::m_marks.size() != 0)
	{
		...
		POINT point2 = {(int)(mouseOverPoint.x - x1), (int)(mouseOverPoint.y - y1)};
...
ActionType P011::GetAction()
{
	...
	POINTF firstP = { x2, y2 - (float)((y2 - y1) * 0.382) };
	POINTF secondP = { x2, y2 - (float)((y2 - y1) * 0.5)};
	POINTF thirdP = { x2, y2 - (float)((y2 - y1) * 0.618)};
	POINTF startP = { x1, y1};
	if (!CPlotBaseMe::SelectSegment(mouseOverPoint, x1, y1, x2, y2))
	{
...
ActionType P014::GetClickStatus()
{
	...
	if (((mouseOverPoint.x >= bigRect.left) && (mouseOverPoint.x <= bigRect.right)) && ((mouseOverPoint.y >= bigRect.top) && (mouseOverPoint.y <= bigRect.bottom)))
	{
		...
		//int count = sizeof(listP) / sizeof(POINT);
		for (int i = 0; i < length; i++)
		{
		...
		//count = sizeof(listP) / sizeof(POINT);
...
POINT* P014::GetGannBoxPoints(float x1, float y1, float x2, float y2, int *length)
{
	...
	POINT thirdP = {(int)x2, (int)(y2 - (float)((y2 - y1) * 0.67))};
	POINT forthP = {(int)x2, (int)(y2 - (float)((y2 - y1) * 0.5))};
	POINT fifthP = {(int)(x2 - ((x2 - x1) * 0.875)), (int)y2};
	POINT sixthP = {(int)(x2 - ((x2 - x1) * 0.75)), (int)y2};
	POINT seventhP = {(int)(x2 - ((x2 - x1) * 0.67)), (int)y2};
	POINT eighthP = {(int)(x2 - ((x2 - x1) * 0.5)), (int)y2};
	*length = 8;
...
void P014::OnMoveStart()
{
	...
	case ActionType_AT2:
		...
		point.y = y3;
		m_oppositePoint = point;
		//m_oppositePoint = new POINT(x3, y3);
		break;
	...
	case ActionType_AT3:
		...
		point.y = y2;
		m_oppositePoint = point;
		//m_oppositePoint = new POINT(x2, y2);
		break;
	...
	case ActionType_AT4:
		...
		point.y = y1;
		m_oppositePoint = point;
		//m_oppositePoint = new POINT(x1, y1);
		break;
	}
	CPlotBaseMe::ClearMarks(&m_startMarks);
	CPlotBaseMe::m_startPoint = CPlotBaseMe::GetMouseOverPoint();
...
ActionType P016::GetAction()
{
	...
	if ((int)CPlotBaseMe::m_marks.size() != 0)
	{
		...
		POINTF firstP = {x2, (float)(y2 - ((y2 - y1) * 0.875))};
		POINTF secondP = {x2, (float)(y2 - ((y2 - y1) * 0.75))};
		POINTF thirdP = {x2, (float)(y2 - ((y2 - y1) * 0.67))};
		POINTF forthP = {x2, (float)(y2 - ((y2 - y1) * 0.5))};
		POINTF fifthP = {(float)(x2 - ((x2 - x1) * 0.875)), y2};
		POINTF sixthP = {(float)(x2 - ((x2 - x1) * 0.75)), y2};
		POINTF seventhP = {(float)(x2 - ((x2 - x1) * 0.67)), y2};
		POINTF eighthP = {(float)(x2 - ((x2 - x1) * 0.5)), y2};
		POINTF startP = {x1, y1};
		vector<POINTF> listP;
		POINTF *collection = new POINTF[8];
		//{ firstP, secondP, thirdP, forthP, fifthP, sixthP, seventhP, eighthP };
		collection[0] = firstP;
		listP.push_back(firstP);
		collection[1] = secondP;
		listP.push_back(secondP);
...
void P016::Paint(CPaintMe *paint, map<int,CPlotMarkMe*> *pList, _int64 lineColor)
{
	if ((int)pList->size() != 0)
	{
		...
		{
			POINTF firstP = {x2, (float)(y2 - ((y2 - y1) * 0.875))};
            POINTF secondP = {x2, (float)(y2 - ((y2 - y1) * 0.75f))};
			POINTF thirdP = {x2, (float)(y2 - ((y2 - y1) * 0.67f))};
			POINTF forthP = {x2, (float)(y2 - ((y2 - y1) * 0.5f))};
			POINTF fifthP = {(float)(x2 - ((x2 - x1) * 0.875f)), y2};
			POINTF sixthP = {(float)(x2 - ((x2 - x1) * 0.75f)), y2};
			POINTF seventhP = {(float)(x2 - ((x2 - x1) * 0.67f)), y2};
			POINTF eighthP = {(float)(x2 - ((x2 - x1) * 0.5f)), y2};
			POINTF startP = {x1, y1};
			//List<POINTF> listP = new List<POINTF>();
			//POINTF[] collection = new POINTF[] { firstP, secondP, thirdP, forthP, fifthP, sixthP, seventhP, eighthP };
			//listP.AddRange(collection);
			vector<POINTF> listP;
			POINTF *collection = new POINTF[8];
			//{ firstP, secondP, thirdP, forthP, fifthP, sixthP, seventhP, eighthP };
...
void P017::Paint(CPaintMe *paint, map<int,CPlotMarkMe*> *pList, _int64 lineColor)
{
	if ((int)pList->size() != 0)
	{
		...
		//int length = sizeof(lineParam) / sizeof(float);
		for (int i = 0; i < length; i++)
		{
	...
	if ((int)pList->size() != 0)
	{
		...
		if ((x2 - x1) != 0)
		{
			...
			*length = 3;
			...
			ret[2] = num12;
			return ret;
			//return new float[] { a, num11, num12 };
		}
	}
	return 0;
}
...
ActionType P028::GetAction()
{
	...
	if ((int)CPlotBaseMe::m_marks.size() != 0)
	{
		...
		float x1 = CPlotBaseMe::PX(aIndex);
		float y1 = CPlotBaseMe::PY(fValue);
		RECT rect = {(int)x1, (int)y1, (int)(x1 + m_textSize.cx), (int)(y1 + m_textSize.cy)};
...
void P035::Paint(CPaintMe *paint, map<int,CPlotMarkMe*> *pList, _int64 lineColor)
{
	...
	if ((int)pList->size() != 0)
	{
		...
		if ((x1 != x2) && (y1 != y2))
		{
			POINT firstP = {(int)x2, (int)(y2 - ((y2 - y1) / 3))};
			POINT secondP = {(int)x2, (int)(y2 - (((y2 - y1) * 2) / 3))};
			POINT startP = {(int)x1, (int)y1};
			float fK = 0;
			float fB = 0;
			float sK = 0;
			float sB = 0;
...
ActionType P043::GetAction()
{
	...
	if ((int)CPlotBaseMe::m_marks.size() != 0)
	{
		...
		float y1 = CPlotBaseMe::PY(fValue);
		int width = 10;
		RECT rect = {(int)(x1 - (width / 2)), (int)y1, (int)(x1 + (width / 2)), (int)(y1 + ((width * 3) / 2))};
		POINT mouseOverPoint = CPlotBaseMe::GetMouseOverPoint();
...
void P043::Paint(CPaintMe *paint, map<int,CPlotMarkMe*> *pList, _int64 lineColor)
{
	if ((int)pList->size() != 0)
	{
		...
		points[5] = point6;
		points[6] = point7;
		//= new POINT[] { point, point2, point3, point4, point5, point6, point7 };
		CPlotBaseMe::FillPolygon(paint, lineColor, points, 7);
		if (IsSelected())
		{
	...
	*length = 0;
	...
	if (candleRange != NULL && *length > 0)
	{
		...
		*length = 5;
		...
		retArray[4] = nLow;
		//return new double[] { nHigh, , , (nHigh - ((2.0 * (nHigh - nLow)) / 3.0)), nLow };
	}
	return NULL;
}
...
ActionType P051::GetAction()
{
	...
	if ((int)CPlotBaseMe::m_marks.size() != 0)
	{
		...
		POINT p = {(int)(mp.x - x1), (int)(mp.y - y1)};
